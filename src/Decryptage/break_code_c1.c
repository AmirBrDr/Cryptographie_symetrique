...
void break_code_c1(const unsigned char *message_entree, size_t taille_cle, char **cle_possibles, int *nombre_cle, int taille_contenu){
    int total_cle = 0;
    int compteur;
    ...
    for(int index1 = 0; index1 < taille_cle; index1++){
        ...
        for(int index2 = 0; index2 < maxCaracteres; index2++){
            if(charCorrect(message_entree, alphabet[index2], tailleClef, index1, taille_contenu)){
                cle_possibles[index1][compteur] = alphabet[index2];
                compteur++;
                total_cle++;
            }
        }
    ...
    *nombre_cle = total_cle;
    ...
    //construction du tableau de clé
}
